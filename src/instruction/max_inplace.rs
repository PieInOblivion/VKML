...
#[derive(Clone)]
pub struct MaxInplaceInstruction {
    pub dst: TensorId,
    pub src1: TensorId,
}

impl Debug for MaxInplaceInstruction {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        write!(f, "MaxInplace(dst={}, src1={})", self.dst, self.src1)
    }
}

impl Instruction for MaxInplaceInstruction {
    ...
    fn create_command_buffer(
        ...
        let bc = TensorDesc::broadcast_shape(&dims_a, &dims_b).expect(&format!(
            "InplaceMax: can't broadcast {:?} vs {:?}",
            dims_a, dims_b
        ));

        assert_eq!(
            bc, dims_a,
            "InplaceMax: broadcast {:?} != out {:?}",
            bc, dims_a
        );
        ...
        assert!(
            rank <= 8,
            "InplaceMax: tensor rank {} exceeds maximum supported rank of 8",
            rank
        );
        ...
        unsafe {
            ...
            let pipeline = gpu
                ...
                .get_pipeline(GPUMemoryOperation::MaximumInplace)
                .ok_or(format!("MaximumInplace pipeline not found"))?;
    ...
    fn execute_cpu(&self, tensor_graph: Arc<TensorGraph>) {
        ...
        for i in 0..data_a.len() {
            ...
            data_a[offa] = data_a[offa].max(data_b[offb]);
        }
    }
}
