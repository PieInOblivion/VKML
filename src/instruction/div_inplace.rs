...
#[derive(Clone)]
pub struct DivInplaceInstruction {
    pub dst: TensorId,
    pub src1: TensorId,
}

impl Debug for DivInplaceInstruction {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        write!(f, "DivInplace(dst={}, src1={})", self.dst, self.src1)
    }
}

impl Instruction for DivInplaceInstruction {
    ...
    fn create_command_buffer(
        ...
        let bc = TensorDesc::broadcast_shape(&dims_a, &dims_b).expect(&format!(
            "InplaceDiv: can't broadcast {:?} vs {:?}",
            dims_a, dims_b
        ));

        assert_eq!(
            bc, dims_a,
            "InplaceDiv: broadcast {:?} != out {:?}",
            bc, dims_a
        );
        ...
        assert!(
            rank <= 8,
            "InplaceDiv: tensor rank {} exceeds maximum supported rank of 8",
            rank
        );
        ...
        unsafe {
            ...
            let pipeline = gpu
                ...
                .get_pipeline(GPUMemoryOperation::DivideInplace)
                .ok_or(format!("DivideInplace pipeline not found"))?;
